...
function makeHttp() {
  ...
  const activity = Activity.layer.pipe(Layer.provideMerge(deps))
  const proc = SessionProcessor.layer.pipe(Layer.provide(SessionSummary.defaultLayer), Layer.provideMerge(activity), Layer.provideMerge(deps))
